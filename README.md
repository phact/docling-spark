...
data movement. The package depends on the service-client, conversion-core, and
chunking extras from `docling-slim`; executors do not install or load Docling
conversion models.
...
| `docling_chunks` | array | Contextualized text and Docling chunk metadata |
...
execution is at least once: if an executor converts a document and fails before
committing its output, a task retry can submit the document again. This does not
duplicate successfully committed Delta rows, but it can repeat paid conversion.
